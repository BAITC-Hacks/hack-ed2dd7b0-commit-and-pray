...
| `LLM_API_KEY_BACKUP` | Резервный ключ NVIDIA NIM | Нет |
| `LLM_BASE_URL_BACKUP` | Резервный OpenAI-совместимый endpoint | Нет |
| `LLM_MODEL_BACKUP` | Резервная модель NVIDIA | Нет |
...
После успешного расчёта кнопка «Что можно улучшить?» вызывает `POST /api/recommend`.
LLM предлагает только альтернативные меры, а валидатор и `compute_score()`
повторно проверяют каждый набор; отображаются только валидные варианты с
улучшенным Score. Кнопка «Сохранить сценарий для сравнения» хранит последние
три результата в `localStorage` браузера.
