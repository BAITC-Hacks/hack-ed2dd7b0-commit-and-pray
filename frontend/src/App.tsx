...
export default function App() {
  ...
  if (!districts || !measuresData || baseScore === null) {
    return <div className="min-h-screen bg-[#0b0f19] p-6"><div className="mx-auto max-w-6xl animate-pulse space-y-6"><div className="h-24 rounded-2xl bg-slate-800/70" /><div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-64 rounded-2xl bg-slate-800/60" />)}</div><div className="space-y-3">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-16 rounded-2xl bg-slate-800/60" />)}</div></div></div></div>
  }
  ...
  const applyRecommendation = (recommendation: Recommendation) => {
    ...
    window.setTimeout(() => setToast(`Сценарий применён: выбрано ${recommendation.selections.length} мероприятий, бюджет ${recommendation.selections.reduce((sum, item) => sum + (measuresData?.measures[item.measure_id]?.cost ?? 0), 0)}/100`), 0)
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19]">
      <header className="relative overflow-hidden border-b border-slate-800 px-4 sm:px-6 py-6 flex flex-col items-center text-center sm:flex-row sm:items-center sm:text-left justify-between gap-3 sm:gap-4">
        ...
        <motion.div key={result?.valid && result.score && result.score > baseScore ? "improved" : "steady"} initial={result?.valid && result.score !== undefined && result.score > baseScore ? { boxShadow: "0 0 0 rgba(52,211,153,0)" } : undefined} animate={result?.valid && result.score !== undefined && result.score > baseScore ? { boxShadow: ["0 0 0 rgba(52,211,153,0)", "0 0 28px rgba(52,211,153,0.45)", "0 0 0 rgba(52,211,153,0)"] } : undefined} transition={{ duration: 0.6 }} className="relative shrink-0 rounded-full scale-90 sm:scale-100"><QoLGauge score={result?.valid ? (result.score ?? baseScore) : baseScore} baseScore={baseScore} /></motion.div>
      </header>

      <div className="sticky top-0 z-20 flex border-b border-slate-800 bg-[#0b0f19]/95 p-2 lg:hidden">
        <button onClick={() => setMobileTab("measures")} className={`flex-1 rounded-2xl py-2 text-sm font-semibold transition-all duration-150 hover:scale-[1.01] ${mobileTab === "measures" ? "bg-violet-600 text-white" : "text-slate-400"}`}>Мероприятия</button>
        <button onClick={() => setMobileTab("city")} className={`flex-1 rounded-2xl py-2 text-sm font-semibold transition-all duration-150 hover:scale-[1.01] ${mobileTab === "city" ? "bg-violet-600 text-white" : "text-slate-400"}`}>Город</button>
      </div>
      <main className="px-4 sm:px-6 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <AnimatePresence mode="wait" initial={false}>{mobileTab === "city" || isDesktop ? <motion.section key="city" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.2 }} className="lg:col-span-2 space-y-4">
          <h2 className="text-lg font-semibold text-slate-100">Районы города</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {Object.entries(districts).map(([id, d], index) => (
              <motion.div key={id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                <DistrictCard
                  district={d}
          ...
          <AnimatePresence mode="wait">{result && <motion.div key="verdict" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.2 }}><VerdictPanel result={result} districts={districts} recommendations={recommendations} recommendationsLoading={recommendationsLoading} onApplyRecommendation={applyRecommendation} /></motion.div>}</AnimatePresence>
        </motion.section> : null}</AnimatePresence>

        <AnimatePresence mode="wait" initial={false}>{mobileTab === "measures" || isDesktop ? <motion.aside key="measures" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.2 }} className="space-y-4">
          <MeasureSelector
      ...
      <AnimatePresence>{toast && <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 24 }} className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 rounded-2xl border border-emerald-400/40 bg-slate-900 px-4 py-3 text-sm text-emerald-200 shadow-2xl">{toast}</motion.div>}</AnimatePresence>
    </div>
  )
}
